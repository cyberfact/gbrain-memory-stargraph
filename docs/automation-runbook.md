...
Recurring worker Run/report persistence uses explicit top-level `curl`
commands generated by offline preparation. `scripts/automation/worker_persistence.py
prepare <health|read|save|tags> ... --json` reads local private route config,
prefers the configured non-loopback dashboard route such as
`MEMORY_STARGRAPH_DASHBOARD_URL`, writes safe JSON payload files, encodes slugs,
and emits deterministic `curl` argv/shell command lines plus offline verifier
commands. The helper must not perform network transport, route probing, or
subprocess curl. The worker runs each emitted `curl` command as its own visible
top-level command, then runs the emitted offline verifier against the raw JSON
readback file. If no non-loopback dashboard route is configured, preparation
fails closed instead of silently falling back to loopback. Do not use bash
scripts that call curl, Python sockets, Python-spawned curl, direct `gbrain`,
loopback, or PostgreSQL fallback for recurring-worker persistence transport.
Preserve bounded retries by repeating the emitted explicit top-level curl
command up to the reported retry count with the reported timeout.
